...
type MonitorServer = { id: string; name: string; status: string; cpu: number; memory: number; disk: number; latency: number; sampledAt?: string; services?: unknown[] };
...
function toMonitoring(input: BackendMonitoring): MonitoringData {
  return {
    servers: input.servers.map((server) => ({ id: server.serverId, name: server.serverName, status: server.status ?? server.connectionStatus, cpu: Number(server.cpuPercent ?? 0), memory: Number(server.memoryPercent ?? 0), disk: Number(server.diskPercent ?? 0), latency: Number(server.latencyMs ?? 0), sampledAt: server.sampledAt, services: server.services })),
