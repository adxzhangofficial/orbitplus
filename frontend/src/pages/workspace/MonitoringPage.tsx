...
export function MonitoringPage() {
  const resource = useLiveResource({ servers: [], alerts: [] } as MonitoringData, async () => toMonitoring(await api.get<BackendMonitoring>("/monitoring")), 15000);
  ...
  const [probe, probePending] = useAction(probeRequest);
  const [updateAlert, updateAlertPending] = useAction(updateAlertRequest);
  ...
  async function updateAlertRequest(alert: MonitorAlert, status: "acknowledged" | "resolved") {
    if (live) {
