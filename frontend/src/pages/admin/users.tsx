...
export function UsersAdminPage() {
  ...
  return <>
    ...
    <Drawer open={Boolean(selected)} onClose={() => setSelectedId(undefined)} title={selected?.name ?? "User"} description={selected?.email} footer={selected && <><AdminButton variant="danger" onClick={() => unsupported("User suspension and session revocation")}>Suspend & revoke <small>Unsupported</small></AdminButton><AdminButton onClick={() => unsupported("Secure sign-in link delivery")}>Send sign-in link <small>Unsupported</small></AdminButton></>}>
      {selected && <><div className="flex items-center gap-3 mb-4"><Avatar name={selected.name} /><div className="adm-cell-copy"><b>{selected.name}</b><small>{selected.organization}</small></div><span className="ml-auto"><StatusPill status={selected.status} /></span></div><DetailGrid items={[["Role", selected.role], ["Organization ID", <span className="adm-mono">{selected.organizationId}</span>], ["Membership status", selected.status], ["Joined", new Date(selected.joinedAt).toLocaleDateString()], ["MFA", "Not exposed by admin API"], ["Active sessions", "Not exposed by admin API"]]} />
        <p className="adm-section-label">Access controls</p><div className="adm-check-row"><span><b>Change tenant role</b><small>Requires a platform membership mutation endpoint.</small></span><IconAction label="Change role unsupported" onClick={() => unsupported("Tenant role update")}><KeyRound /></IconAction></div><div className="adm-check-row"><span><b>Reset authentication factors</b><small>Requires an identity-security endpoint.</small></span><IconAction label="Reset MFA unsupported" onClick={() => unsupported("Authentication factor reset")}><KeyRound /></IconAction></div><div className="adm-check-row"><span><b>Revoke active sessions</b><small>Requires a platform session endpoint.</small></span><IconAction label="Revoke sessions unsupported" onClick={() => unsupported("Session revocation")}><LockKeyhole /></IconAction></div><div className="adm-notice mt-4"><ShieldCheck />Identity rows are live membership records. Authentication posture and access mutations are intentionally labeled unavailable because the current backend does not expose those platform-admin contracts.</div>
      </>}
    </Drawer>
  </>;
}
