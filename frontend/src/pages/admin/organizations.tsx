...
export function OrganizationsAdminPage() {
  ...
  return <>
    ...
    <Drawer open={Boolean(selected)} onClose={() => { setSelectedId(undefined); setDetail(undefined); }} title={selected?.name ?? "Organization"} description={selected ? `${selected.slug}.orbit.run · ${selected.id}` : undefined} footer={selected && <><AdminButton variant="danger" disabled={mutating} onClick={() => void patchCustomer({ status: selected.status === "suspended" ? "active" : "suspended" }, selected.status === "suspended" ? `${selected.name} access restored` : `${selected.name} suspended`)}>{selected.status === "suspended" ? "Restore access" : "Suspend tenant"}</AdminButton><AdminButton onClick={() => unsupported("Customer workspace impersonation")}><ExternalLink />Open workspace <small>Unsupported</small></AdminButton></>}>
